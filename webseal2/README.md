...
```bash
npm install @sapcy/webseal
# 또는
yarn add @sapcy/webseal
```
...
```tsx
import { useWebSeal, WebSealForm, WebSealOutput } from '@sapcy/webseal/components'
...
```ts
import { sealSecrets } from '@sapcy/webseal/api'
...
```ts
import { parsePublicKey, sealData, generateSealedSecretYAML } from '@sapcy/webseal/api'
